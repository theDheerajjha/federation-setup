...
export default {
  ...
  watch: {
    // Watch for changes in appName prop
    appName: {
      handler(newAppName, oldAppName) {
        console.log(`App name changed from ${oldAppName} to ${newAppName}`)
        // Always reload when appName changes
        this.loadRemoteApp()
      },
      immediate: true
    },
      // Watch for route changes (query parameters)
    '$route.query': {
      handler(newQuery, oldQuery) {
        console.log('Route query changed:', newQuery)
        // Reload iframe when query parameters change
        this.loadRemoteApp()
      },
      deep: true
    },
    // Watch for store changes to sync with iframe
    'store.users': {
      handler(newUsers) {
  ...
  methods: {
    ...
    onIframeLoad() {
      ...
      // You can add communication between parent and iframe here if needed
